...
public interface EventRepository extends CrudRepository<Event, String> {
    ...
    @Query(value = "select * from EVENT_REQUEST where union_id = (:param1) AND shift_id = (:param2) AND event_status != 'pending' AND event_status != 'declined'", nativeQuery = true)
    ...
    @Query(value = "select COUNT(DISTINCT officer_id) from EVENT_REQUEST where union_id = (:param1) AND event_status != 'pending' AND event_status != 'declined' AND start_time <= (:param2) AND end_time >= (:param2) AND shift_id = (:param3)", nativeQuery = true)
    Integer findByAllDate(@Param("param1") int unionid, @Param("param2") String date, @Param("param3") int shiftType);
